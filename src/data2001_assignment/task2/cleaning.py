...
def normalise_coordinate(value: Any) -> float:
    """统一坐标浮点表达, 避免 API 返回 1e-14 级尾差."""
    return round(float(value), 12)


def clean_poi_feature(settings: Settings, feature: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "longitude": normalise_coordinate(longitude),
        "latitude": normalise_coordinate(latitude),
    }


def parse_population_feature(settings: Settings, feature: dict[str, Any]) -> dict[str, Any]:
